...
##################################################################### 
cvsFileName='BASCII LN2 Calibration testOut_MW_test'
FromCVSFile=True
##################################################################### 
...
for ChannelNameNumber in ChannelsToAnalyze:   
    if np.mod(ChannelNameNumber,4)==1:
        ...
        pl.plot(TemperatureSwept, A*TemperatureSwept+B,label='%.4f*T+%.2f, R:%.3f %%'%(A,B,np.sqrt(residuals/L)*100), linewidth = 2.0,color='red')
        [A,B], residuals, rank, s= np.linalg.lstsq(XcoordmeanRadTemperatureVector, normalizedNDD2Vector) # your data x, y to fit     
        pl.plot(TemperatureSwept, A*TemperatureSwept+B,label='%.4f*T+%.2f, R:%.3f %%'%(A,B,np.sqrt(residuals/L)*100), linewidth = 2.0,color='blue')
        [A,B], residuals, rank, s= np.linalg.lstsq(XcoordmeanRadTemperatureVector, normalizedNDD3Vector) # your data x, y to fit       
        pl.plot(TemperatureSwept, A*TemperatureSwept+B,label='%.4f*T+%.2f, R:%.3f %%'%(A,B,np.sqrt(residuals/L)*100), linewidth = 2.0,color='green')  
        pl.grid(True)
        pl.ylim([0.65, 1.5])    
        pl.xlabel('Tph receiver [C]')
        ...
        pl.plot(TemperatureSwept, A1*TemperatureSwept+B1,label='%.4f*T+%.2f, R:%.2f K'%(A1,B1,np.sqrt(residuals/L)), linewidth = 2.0,color='red')
        [A2,B2], residuals, rank, s= np.linalg.lstsq(XcoordNSTemperature, TN2Vector) # your data x, y to fit          
        pl.plot(TemperatureSwept, A2*TemperatureSwept+B2,label='%.4f*T+%.2f, R:%.2f K'%(A2,B2,np.sqrt(residuals/L)), linewidth = 2.0,color='blue')
        [A3,B3], residuals, rank, s= np.linalg.lstsq(XcoordNSTemperature,  TN3Vector) # your data x, y to fit      
        pl.plot(TemperatureSwept, A3*TemperatureSwept+B3,label='%.4f*T+%.2f, R:%.2f K'%(A3,B3,np.sqrt(residuals/L)), linewidth = 2.0,color='green')  
        pl.grid(True)
        pl.xlabel('Tph NS [C]')
        pl.ylabel('$T_{NS}$ [K]')
        ...
        pl.plot(TemperatureSwept, A1*TemperatureSwept+B1,label='%.4f*T+%.2f, R:%.3f %%'%(A1,B1,np.sqrt(residuals/L)*100), linewidth = 2.0,color='red')
        [A2,B2], residuals, rank, s= np.linalg.lstsq(XcoordNSTemperature, normalizedNDD2Vector) # your data x, y to fit          
        pl.plot(TemperatureSwept, A2*TemperatureSwept+B2,label='%.4f*T+%.2f, R:%.3f %%'%(A2,B2,np.sqrt(residuals/L)*100), linewidth = 2.0,color='blue')
        [A3,B3], residuals, rank, s= np.linalg.lstsq(XcoordNSTemperature,  normalizedNDD3Vector) # your data x, y to fit      
        pl.plot(TemperatureSwept, A3*TemperatureSwept+B3,label='%.4f*T+%.2f, R:%.3f %%'%(A3,B3,np.sqrt(residuals/L)*100), linewidth = 2.0,color='green')  
        pl.grid(True)
        pl.xlabel('Tph NS [C]')
        ...
        pl.plot(NSTemperatureVectorSwept, A3*NSTemperatureVectorSwept+B3,label='%.4f*$T_{ph} NS$+%.2f, R:%.3f %%'%(A3,B3,np.sqrt(residuals/L)*100), linewidth = 2.0,color='green')      
        pl.grid(True)
        pl.xlabel('Tph NS [C]')
        pl.ylabel('$NDDR_{X1}$ ')
