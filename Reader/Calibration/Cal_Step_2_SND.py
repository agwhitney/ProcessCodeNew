...
pathfordata=os.path.join(pathfordata, t+'_'+cvsFileName+'_Trec_model_'+RadiometricDataType+'.csv')
...
##################################################################### 
cvsFileName='ln2Out_SND_All_v005'
FromCVSFile=True
##################################################################### 
pathfordata=os.path.join(L0b_DATA_PROCESSED_PATH, 'ln2CVS',  cvsFileName+'.csv')
...
for row in spamreader:
    ...
    preMultuiplierTemperatureVector.append(row[15])  # ADAM changed to 15 from 16 (horn temp)
...
for ChannelNameNumber in ChannelsToAnalyze:   
    ...
    pathfordata = 'D:\\OneDrive - Colostate\\Research - Radiometry\\MiniMAC\\HAMMR measurements\\HAMMR metadata\\ProcessCode\\Reader\\Results\\L0bdata\\ln2CVS'    
    # pathfordata = r"C:\Users\adamgw\OneDrive - Colostate\Research - Radiometry\MiniMAC\HAMMR\HAMMR metadata\ProcessCode\Reader\Results\L0bdata\cooktop_ln2CVS"
    ...
    pl.show(block=False) 
    figfilename = "Figure 2300 CalStep2"  # ADAM: Namelength error
    # os.path.join(pathfordata,filenameroot+'_'+str(fignum)+'_'+title.replace(" ", "_")+'.png')
    fig.savefig(figfilename + str(ChannelNameNumber))
    ...
    pl.legend(loc = 'best',shadow = True)
    pl.show(block=False) 
    figfilename = "2400" # os.path.join(pathfordata,filenameroot+'_'+str(fignum)+'_'+title.replace(" ", "_")+'.png')
    fig.savefig(figfilename + str(ChannelNameNumber))    
    ...
    pl.legend(loc = 'best',shadow = True)
    pl.show(block=False) 
    figfilename ="2401" # os.path.join(pathfordata,filenameroot+'_'+str(fignum)+'_'+title.replace(" ", "_")+'.png')
    fig.savefig(figfilename + str(ChannelNameNumber))         
  ...
  #  pl.ylim([-2.6, 2.6])
    ...
    figfilename = "2301" # os.path.join(pathfordata,filenameroot+'_'+str(fignum)+'_'+ 'NormGainErrAfterTempTimeComp' + ChannelName +'.png')
    fig.savefig(figfilename + str(ChannelNameNumber))
    ...
    try:
        ...
        fig.colorbar(surf)
    except ValueError:  # This seems to happen if something is all NaN somewhere... in the p1 output?
        exceptionCount += 1
        continue
    ...
    pl.legend(loc = 'best',shadow = True)
    pl.show(block=False) 
    figfilename = "92401" # os.path.join(pathfordata,filenameroot+'_'+str(fignum)+'_'+title.replace(" ", "_")+'.png')
    fig.savefig(figfilename + str(ChannelNameNumber))         
