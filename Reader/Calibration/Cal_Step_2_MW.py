...
pathfordata = Path(L0b_DATA_PROCESSED_PATH) / f'{ln2_filename}CVS'  # AGW I think this gets redefined a lot...
...
CHANNELS = [
    Channel('MW_34_GHz_QV', 0),
    Channel('NotConnected', 1),
    Channel('MW_18_GHz_QV', 2),
    Channel('MW_24_GHz_QV', 3),
    Channel('MW_34_GHz_QH', 4),
    Channel('NotConnected', 5),
    Channel('MW_18_GHz_QH', 6),
    Channel('MW_24_GHz_QH', 7),
]
...
def calstep2():
    ...
    ChannelsNames = ['MW_34_GHz_QV','NotConnected', 'MW_18_GHz_QV', 'MW_24_GHz_QV','MW_34_GHz_QH','NotConnected', 'MW_18_GHz_QH', 'MW_24_GHz_QH']
    ChannelsToAnalyze = [0, 1, 2, 3, 4, 5, 6, 7]
    ...
    for ChannelNameNumber in ChannelsToAnalyze:
        if np.mod(ChannelNameNumber, 4) == 1:
            ...
            FilenameVector = [ preFilenameVector[i] for i in indexes ]
            GPSTimeVector = [ float(preGPSTimeVector[i]) for i in indexes ]
            GainVector = [ float(preGainVector[i]) for i in indexes ]
            TrecVector = [ float(preTrecVector[i]) for i in indexes ]
            AltitudeVector = [ float(preAltitudeVector[i]) for i in indexes ]
            LatVector = [ float(preLatVector[i]) for i in indexes ]
            LonVector = [ float(preLonVector[i]) for i in indexes ]
            ExternalTemperatureVector = [ float(preExternalTemperatureVector[i]) for i in indexes ]
            meanRadTemperatureVector = [ float(premeanRadTemperatureVector[i]) for i in indexes ]
            stdRadTemperatureVector = [ float(prestdRadTemperatureVector[i]) for i in indexes ]
            TN1Vector = [ float(preTN1Vector[i]) for i in indexes ]
            TN2Vector = [ float(preTN2Vector[i]) for i in indexes ]
            TN3Vector = [ float(preTN3Vector[i]) for i in indexes ]
            NDD1Vector = [ float(preNDD1Vector[i]) for i in indexes ]
            NDD2Vector = [ float(preNDD2Vector[i]) for i in indexes ]
            NDD3Vector = [ float(preNDD3Vector[i]) for i in indexes ]
            
            NS1824TemperatureVector = [ float(preNS1824TemperatureVector[i]) for i in indexes ]
            NS34TemperatureVector = [ float(preNS34TemperatureVector[i]) for i in indexes ]    
            ...
            plt.plot(meanRadTemperatureVector,GainVector, marker='o', linestyle = 'None', label='Trec',color='blue')
            ...
            plt.plot(TemperatureSwept, Ag2*TemperatureSwept+Bg2,label='%.4f*T+%.2f, R:%.3f K'%(Ag2,Bg2,R[0]), linewidth = 2.0,color='black')                
            plt.grid(True)
            plt.xlabel('Tph receiver [C]')
            plt.ylabel('Gain [K/v]')
            ...
            plt.plot(meanRadTemperatureVector,normalizedGain, marker='o', label='Calibration Gain', linestyle = 'None',color='red')                     
            ...
            Bgain.append(Bg)
            Rgain.append(R[0])    
            plt.plot(TemperatureSwept, Ag*TemperatureSwept+Bg,label='%.4f*T+%.2f, R:%.3f%%'%(Ag,Bg,R[0]), linewidth = 2.0,color='black')    
            plt.grid(True)
            plt.ylim([0.6, 1.3])
            plt.xlabel('Tph receiver [C]')
            ...
            plt.plot(meanRadTemperatureVector,normalizedTrecVector, marker='o', linestyle = 'None', label='Trec',color='blue')
            ...
            BTrec.append(Bt)
            RTrec.append(R[0])      
            plt.plot(TemperatureSwept, At*TemperatureSwept+Bt,label='%.4f*T+%.2f, R:%.3f%%'%(At,Bt,R[0]), linewidth = 2.0,color='black')                
            ...
            ax = fig.add_subplot(111)
            plt.plot(meanRadTemperatureVector,TrecVector, marker='o', linestyle = 'None', label='Trec',color='blue')
            ...
            plt.plot(TemperatureSwept, At2*TemperatureSwept+Bt2,label='%.4f*T+%.2f, R:%.3f K'%(At2,Bt2,R[0]), linewidth = 2.0,color='black')                
            ...
            fig = plt.figure(fignum)
            ax = fig.add_subplot(111)       
            plt.plot(Time-Time[0],100*(normalizedGain-(Ag*XvariablemeanRadTemperatureVector+Bg)), marker='o', label='Gain Residual', linestyle = 'None',color='red')
            plt.plot(Time-Time[0],100*(normalizedTrecVector-(At*XvariablemeanRadTemperatureVector+Bt)), marker='o', label='$T_{REC}$ Residual', linestyle = 'None',color='blue')                      
            plt.grid(True)
            plt.ylim([-2.6, 2.6])
            plt.xlabel('Time [s]')
            plt.ylabel('Residual [%]')
            ...
            plt.plot(meanRadTemperatureVector,normalizedNDD1Vector, marker='o', label='$NDD_{NS1}$', linestyle = 'None',color='red')                
            plt.plot(meanRadTemperatureVector,normalizedNDD2Vector, marker='o', label='$NDD_{NS2}$', linestyle = 'None',color='blue')
            plt.plot(meanRadTemperatureVector,normalizedNDD3Vector, marker='o', label='$NDD_{NS3}$', linestyle = 'None',color='green')     
            [A,B], residuals, rank, s= np.linalg.lstsq(XcoordmeanRadTemperatureVector, normalizedNDD1Vector) # your data x, y to fit 
            plt.plot(TemperatureSwept, A*TemperatureSwept+B,label='%.4f*T+%.2f, R:%.3f %%'%(A,B,(np.sqrt(residuals/L)*100)[0]), linewidth = 2.0,color='red')
            [A,B], residuals, rank, s= np.linalg.lstsq(XcoordmeanRadTemperatureVector, normalizedNDD2Vector) # your data x, y to fit     
            plt.plot(TemperatureSwept, A*TemperatureSwept+B,label='%.4f*T+%.2f, R:%.3f %%'%(A,B,(np.sqrt(residuals/L)*100)[0]), linewidth = 2.0,color='blue')
            [A,B], residuals, rank, s= np.linalg.lstsq(XcoordmeanRadTemperatureVector, normalizedNDD3Vector) # your data x, y to fit       
            plt.plot(TemperatureSwept, A*TemperatureSwept+B,label='%.4f*T+%.2f, R:%.3f %%'%(A,B,(np.sqrt(residuals/L)*100)[0]), linewidth = 2.0,color='green')  
            plt.grid(True)
            plt.ylim([0.65, 1.5])    
            plt.xlabel('Tph receiver [C]')
            plt.ylabel('Normalized $NDD_{NSx}$ ')
            ...
            plt.plot(NSTemperature,TN1Vector, marker='o', label='$T_{NS1}$', linestyle = 'None',color='red')                
            plt.plot(NSTemperature,TN2Vector, marker='o', label='$T_{NS2}$', linestyle = 'None',color='blue')
            plt.plot(NSTemperature,TN3Vector, marker='o', label='$T_{NS3}$', linestyle = 'None',color='green')    
            ...
            plt.plot(TemperatureSwept, A1*TemperatureSwept+B1,label='%.4f*T+%.2f, R:%.2f K'%(A1,B1,res), linewidth = 2.0,color='red')
            [A2,B2], residuals, rank, s= np.linalg.lstsq(XcoordNSTemperature, TN2Vector) # your data x, y to fit
            res = np.sqrt(residuals/L)[0] if len(residuals) == 1 else 0        
            plt.plot(TemperatureSwept, A2*TemperatureSwept+B2,label='%.4f*T+%.2f, R:%.2f K'%(A2,B2,res), linewidth = 2.0,color='blue')
            [A3,B3], residuals, rank, s= np.linalg.lstsq(XcoordNSTemperature,  TN3Vector) # your data x, y to fit 
            res = np.sqrt(residuals/L)[0] if len(residuals) == 1 else 0     
            plt.plot(TemperatureSwept, A3*TemperatureSwept+B3,label='%.4f*T+%.2f, R:%.2f K'%(A3,B3,res), linewidth = 2.0,color='green')  
            plt.grid(True)
            plt.xlabel('Tph NS [C]')
            plt.ylabel('$T_{NS}$ [K]')
            ...
            ax = fig.add_subplot(111)
            XcoordNSTemperature = np.vstack([NSTemperature, np.ones(len(NSTemperature))]).T                   
            plt.plot(NSTemperature,normalizedNDD1Vector, marker='o', label='$NDD_{NS1}$', linestyle = 'None',color='red')                
            plt.plot(NSTemperature,normalizedNDD2Vector, marker='o', label='$NDD_{NS2}$', linestyle = 'None',color='blue')
            plt.plot(NSTemperature,normalizedNDD3Vector, marker='o', label='$NDD_{NS3}$', linestyle = 'None',color='green')    
            [A1,B1], residuals, rank, s= np.linalg.lstsq(XcoordNSTemperature, normalizedNDD1Vector) # your data x, y to fit
            res = np.sqrt(residuals/L)[0] if len(residuals) == 1 else 0
            plt.plot(TemperatureSwept, A1*TemperatureSwept+B1,label='%.4f*T+%.2f, R:%.3f %%'%(A1,B1,res), linewidth = 2.0,color='red')
            [A2,B2], residuals, rank, s= np.linalg.lstsq(XcoordNSTemperature, normalizedNDD2Vector) # your data x, y to fit          
            res = np.sqrt(residuals/L)[0] if len(residuals) == 1 else 0
            plt.plot(TemperatureSwept, A2*TemperatureSwept+B2,label='%.4f*T+%.2f, R:%.3f %%'%(A2,B2,res), linewidth = 2.0,color='blue')
            res = np.sqrt(residuals/L)[0] if len(residuals) == 1 else 0
            [A3,B3], residuals, rank, s= np.linalg.lstsq(XcoordNSTemperature,  normalizedNDD3Vector) # your data x, y to fit      
            plt.plot(TemperatureSwept, A3*TemperatureSwept+B3,label='%.4f*T+%.2f, R:%.3f %%'%(A3,B3,res), linewidth = 2.0,color='green')  
            plt.grid(True)
            plt.xlabel('Tph NS [C]')
            plt.ylabel('normalized $NDD_{NS}$')
            ...
            plt.plot(normalizedNDD1Vector, normalizedNDD2Vector , marker='o', label='$NDD_{NS2}$', linestyle = 'None',color='blue')
            plt.plot(normalizedNDD1Vector,  normalizedNDD3Vector , marker='o', label='$(NDD_{NS3})$', linestyle = 'None',color='green')
            ...
            BNDD2.append(B2)
            RNDD2.append(R[0])
            plt.plot(TrecVector2Swept, A2*TrecVector2Swept+B2,label='%.4f*$NDD_{NS1}$+%.2f, R:%.3f %%'%(A2,B2,R[0]), linewidth = 2.0,color='blue')      
            ...
            BNDD3.append(B3)
            RNDD3.append(R[0])
            plt.plot(TrecVector2Swept, A3*TrecVector2Swept+B3,label='%.4f*$NDD_{NS1}$+%.2f, R:%.3f %%'%(A2,B2,R[0]), linewidth = 2.0,color='green')      
            plt.grid(True)
            plt.xlabel('$NDD_{NS1}$')
            plt.ylabel('$NDD_{NSx}$')
            ...
            plt.plot(NSTemperature, ENR2 , marker='o', label='$NDDR_{21}$', linestyle = 'None',color='blue')
            [A2,B2], residualsNDDR2, rank, s= np.linalg.lstsq(XcoordnormalizedTrecVector2Swept,  ENR2) # your data x, y to fit
            res = np.sqrt(residuals/L)[0] if len(residuals) == 1 else 0
            plt.plot(NSTemperatureVectorSwept, A2*NSTemperatureVectorSwept+B2,label='%.4f*$T_{ph} NS$+%.2f, R:%.3f %%'%(A2,B2,res), linewidth = 2.0,color='blue')      
            plt.plot(NSTemperature, ENR3 , marker='o', label='$NDDR_{31}$', linestyle = 'None',color='green')
            [A3,B3], residualsNDDR3, rank, s= np.linalg.lstsq(XcoordnormalizedTrecVector2Swept,  ENR3) # your data x, y to fit
            res = np.sqrt(residuals/L)[0] if len(residuals) == 1 else 0
            plt.plot(NSTemperatureVectorSwept, A3*NSTemperatureVectorSwept+B3,label='%.4f*$T_{ph} NS$+%.2f, R:%.3f %%'%(A3,B3,res), linewidth = 2.0,color='green')      
            plt.grid(True)
            plt.xlabel('Tph NS [C]')
            plt.ylabel('$NDDR_{X1}$ ')
            ...
            plt.plot(meanRadTemperatureVector, ENR2-(A2*NSTemperature2+B2) , marker='o', label='Residuals $NDDR_{21}$', linestyle = 'None',color='blue')
            plt.plot(meanRadTemperatureVector, ENR3-(A3*NSTemperature2+B3) , marker='o', label='Residuals $NDDR_{31}$', linestyle = 'None',color='green')
            plt.grid(True)
            plt.xlabel('Tph receiver [C]')
            plt.ylabel('Residual $NDDR_{x1}$ ')
            ...
            plt.plot(TrecVector, ENR2-(A2*NSTemperature2+B2) , marker='o', label='Residuals $NDDR_{21}$', linestyle = 'None',color='blue')
            plt.plot(TrecVector,ENR3-(A3*NSTemperature2+B3) , marker='o', label='Residuals $NDDR_{31}$', linestyle = 'None',color='green')
            plt.grid(True)
            plt.xlabel('Trec [k]')
            plt.ylabel('Residual $NDDR_{NSx1}$ ')
            ...
            fig = plt.figure(fignum)
            ax = fig.add_subplot(111)                  
            plt.plot(Time,ENR2-(A2*NSTemperature2+B2), marker='o', label='Residuals $NDDR_{21}$', linestyle = 'None',color='blue')
            plt.plot(Time,ENR3-(A3*NSTemperature2+B3), marker='o', label='Residuals $NDDR_{31}$', linestyle = 'None',color='green')
            plt.grid(True)
            plt.xlabel('Time')
            plt.ylabel('Reiduals $NDDR_{x1}$')
