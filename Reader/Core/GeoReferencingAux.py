...
class dataproject:
    ...
    def __init__(self, filenameroot, Channel, pathfordata, RadDataSet, GroundAltitude,  verbose=False):
       # try:        
        ...
        data =eval(datasource)
        #print "INFO: new instance of dataprocessor for ", data  
        self.Altitude= np.array( [x['Altitude'] for x in data.Navigation.iterrows()],'double')-GroundAltitude 
		...
		####################################################################        
        ElipseResolution={'MW':70, 'MMW':30, 'SND':30}
        self.FootprintElipseResolution=ElipseResolution[self.RadiometricSubset]
    	####################################################################
    ...
    def calculateEllipse(self, x, y, a, b, angle, angular_resolution):
	   # calculating the ellipse's contour 
		...
		SpatialResolutionX=self.FootprintElipseResolution
		SpatialResolutionY=self.FootprintElipseResolution
    ...
    def calculateEllipseCenterandAxis(self, Latitude, Longitude, altitude, Zb, Antenna_BeamWidth3dB, verbose=False):
		...
		self.ang=self.ang_footprint(EllipseDirection)
		Antenna_BeamWidth3dBrad=Antenna_BeamWidth3dB*np.pi/180. # in degrees
		self.a=0.5*r*r*Antenna_BeamWidth3dBrad/altitude  #Computing the major semi-axis 
		self.b=0.5*r*Antenna_BeamWidth3dBrad             #Computing the minor semi-axis 
		#self.mayorsemiaxis.append(self.a)
		#self.minorsemiaxis.append(self.b)
		#self.footprintangle.append(self.ang)

		Xe,Ye=self.calculateEllipse(X_, Y_,self.a,self.b, self.ang, 5)
    ...
    def correctingMagneticDeclination(self, hdg, Lat, Lon, Verbose=False):
		...
		## Going East -> chdg=hdg - dec
		## Going West -> chdg=hdg + dec
		#if hdg>0:
		if hdg<-180:
			hdg=hdg+360
		elif hdg>180:
		...
		if hdg>=0 and hdg<= 1*dec:
			dev=dec-1*hdg
		elif hdg>1*dec and hdg<= 2*dec:
			#dev=hdg-dec
			dev=dec-hdg
		elif hdg> 2*dec:
			dev=-dec
		elif hdg<0 and hdg>=-180+2*dec:
 			dev=dec
		elif hdg<-180+2*dec and hdg>=-180+1*dec:
			aux=hdg+180
			dev= (aux-dec)
		elif hdg<-180+1*dec and hdg>=-180:
			aux=hdg+180
			dev=-(dec-1*aux)

		#print "dev and dec:", dev, dec 
		self.dev_vec.append(dev)
		#else:
		chdg=hdg + dev
    ...
    def CorrectFlatReflectorPointingVector(self, pitch, roll, hdg, FlatReflectorPointingVector):
		...
		## For more information read hte IG-500N manual, IG-500N User Manual.pdf
		...
		aaaa=np.array([FlatReflectorPointingVector[0][0], FlatReflectorPointingVector[1][0], FlatReflectorPointingVector[2][0]])  
		#print aaaa
		
		AntennaPointingVector=np.dot(RzRyRx,aaaa)
		...
		self.Az=np.arctan2(AntennaPointingVector[1], AntennaPointingVector[0])
		self.El=np.arccos(AntennaPointingVector[2])
		#self.GroundAz.append(self.Az)
		#self.GroundEl.append(self.El)		
    ...
    def ProjectAntennaTemperature(self,InitialTime=0, FinalTime=298, SceneMaxAngle=45, Verbose=False, PrintWholeEllipse=False):
		...
		if self.Nscans<=FinalTime:
			FinalTime=self.Nscans-1
			print "----> WARNING: Only",self.Nscans, FinalTime, " seconds found in the file. Is this file the first one of a time series acquistion? "
		...
		print FinalTime
		TotaL=round((FinalTime-InitialTime)/5)
		for i in range(InitialTime, FinalTime):

			# Reading Euler angles form the GPS/IMU unit
			...
			if i%TotaL == 0:
				Percent=(100*i/(1.0*FinalTime))
				print self.Channel, " --> Total:", Percent, '%'
				...
				if self.AntennaTemperature[i][ElevationIndex]>0:
				    ...
				    if PrintWholeEllipse==True:
					    for cou in range(0,len(X)):
					    	...
					    	ECenterLato, ECenterLono=utm.to_latlon(np.array(X[cou]), np.array(Y[cou]), UTMZoneNumber, UTMZoneLetter)
					    	self.ECenterLatVector.append(np.array(ECenterLato))
					    	self.ECenterLonVector.append(np.array(ECenterLono))					    	
					    	self.AntennaTemperatureVectorLatLon.append(self.AntennaTemperature[i][ElevationIndex])
					    	self.AntennaTemperatureVector.append(self.AntennaTemperature[i][ElevationIndex])
					    	self.TimeVector.append(self.GPSTime[i])
					    	self.AltitudeVector.append(self.Altitude[i])
					    	self.GroundAz.append(self.Az)
		...
		print len(self.ECenterLonVector), len(self.AntennaTemperatureVector)
		return
