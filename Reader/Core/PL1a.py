...
class P_L1a():
    ...
    def __init__(self, CVSfilenamebatch, TotalSecondsToAnalyzeVector, AngularAverageDegrees, CalibrationFilesNames, ChannelSetToAnalyzeVector, sixtyHertzFilter, Flags):
    ...
    def run(self):
        ...
        font = {'family':'serif','size': 14}    # general font type and sizecsvfileOpenWrite.close() 
        ...
        for CalIndex in range(0,len(self.CVSBatchFiles.filenamerootVector)-1):
            ...
            for ChannelSetToAnalyze in self.ChannelSetToAnalyzeVector:
                ...
                #####################################################################
                if self.DisplayAuxData==True and self.PlotAuxDataTrigger==True: 
                ...
                for x in ChannelsToAnalyze: 
                    ...
                    #mdict={'N1':N1, 'N2':N2,'N3':N3,'N4':N4}
                    #savemat('test', (file_name, mdict, appendmat=True, format='5', long_field_names=False, do_compression=False, oned_as='row'))
                    ...
                    if StrippingCorrection and ChannelSetToAnalyze!='snd':
                        ...
                        for itr in range(NScanns):  
                            ...
                            #if ChannelSetToAnalyze!='SND':
                            #    print  "TN1:", np.mean(N1[itr,CalTargetIndex])*CalCoef[itr], np.std(N1[itr,CalTargetIndex]*CalCoef[itr])
                            #    print  "TN2:", np.mean(N2[itr,CalTargetIndex])*CalCoef[itr], np.std(N2[itr,CalTargetIndex]*CalCoef[itr])
                            ...
                            for itr2 in range(ScannElements[0]):
                                ...
                                #NATA2[itr2,itr]=NA[itr2,itr]*(G1[itr2])-Trec[itr]
                    ...
                    NEAT_1= sqrt(T1_1**2+T2_1**2+T3_1**2+T4_1**2)
                    ...
                    NEAT_2= sqrt(T1_2**2+T2_2**2+T3_2**2+T4_2**2+T5_2**2+T6_2**2)
                    ...
                    #print ChSetdpa.Radiometer.zmin, ChSetdpa.Radiometer.zmax
                    ...
                    #ChSetdpa.Radiometer.AntDataVsScan(9000, x, NATAD, NScanns, self.AngularAverageDegrees, TotalSecondsToAnalyze,GeneralValues=False)
